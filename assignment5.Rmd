...
```{r}
# This data does not look normally distributed, however we have a very large sample size and ANOVA is relatively robust to violations of the normality assumption so we will still use an ANOVA test to compare means


vertebrates_aov <- aov(weight ~ classification, data = vertebrates_2017_vis)
...
# The F value is fairly low

```

<<<<<<< HEAD
=======
The mean weights of Pacific giant salamanders located in pools, cascades, and side channels in 2017 were X, X and X respectively in 2017. Across all three classifications there is a statistally significant difference in mean weights.However, the only statistically significant difference in mean weights in 2017 is between salamanders in side-channels and pools.  
>>>>>>> 4e210d85f8af0d70d22bc5029ba7cc61fe1f6dde
...
The mean weights of Pacific giant salamanders differs significantly between salamanders in pools, cascades, and side-channels in 2017. 

The mean weight of salamanders in cascade, pool and side channel (`r round(sum_table$mean_wt, 2)` grams, respectively).
blah blah blah
